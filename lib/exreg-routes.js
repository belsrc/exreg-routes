...
/**
 * Styles the route capture group.
 * @param  {String} str The route path string.
 * @return {String}
 */
var styleCapture = function(str) {
  var captureRegex = /(\(.+?\))/gi;

  return str.replace(captureRegex, function(match) {
    return chalk.magenta(match);
  });
};
...
module.exports = function(app) {
  ...
  for(var i = 0, len = routes.length; i < len; i++) {
    ...
    if(routes[i].path instanceof RegExp) {
      path = styleCapture(path);
    }
    else {
      path = styleVars(path);
    }
